...
- 检核 commit：`554ba7b`
- 运行时间：2026-07-21 23:42:19
- 运行环境：local/managed python 3.13
- 预测值快照：2026Q3=1.9094 / 2026Q4=1.8606 / 2027Q1=1.8135 / 2027Q2=1.8065
- 实际值快照：2024Q4=2.34 / 2025Q1=2.13 / 2025Q2=1.99 / 2025Q3=1.9 / 2025Q4=1.89 / 2026Q1=1.93 / 2026Q2=1.94

## 执行记录（基于清单模板）

| 检查项 | 结果 | 备注 / 修订 commit |
|--------|------|-------------------|
| C1 预测值 SSOT | ✅ | 两页均引用 predictions.json，无分叉硬编码；当前四值 [1.9094, 1.8606, 1.8135, 1.8065] |
| C2 实际值三处一致 | ⚠️ | formula_report.html 未检索到实际值：2026Q2=1.94（可能为表述差异，需人工复核） |
...
| C9 平推假设 | ✅ | 四季差值 [9.1, 13.9, 18.7, 19.4] BP 均<25，平推（未来 LPR=3.50%/定存=1.30%/债券=1.7385% 维持不变）自洽 |
...
| P6 时间线 | ✅ | actuals 止于 2026Q2，predictions 起于 2026Q3，无错位 |
...
| F5 季度编号 | ✅ | 2024Q4→2027Q2 连续无重复遗漏，实际/预测分界清晰 |
...
- **F6 章节/图编号** ✅ — 公式页第一~第五步编号连续无跳号/重复
- **F7 字段命名** ✅ — 页面读取字段名与 predictions.json/actuals.json 字段名一致，无拼写漂移
- **F8 离线快照时效** ✅ — index.html 离线快照 SNAPSHOT_UPDATED=2026-07-21 23:30 与 predictions.json.last_updated 一致
- **A1 一致性校验** ✅ — check_data_consistency.py 规则1-4 通过（EXIT=0）
- **A2 数据不过期** ✅ — last_updated=2026-07-21 23:30（0.0 天前，≤40）
- **A3 发版后实测** 🔍 — 发版后由独立步骤/手动执行：curl 抓取线上两页，确认无旧值 1.9383、均引用 predictions.json、last_updated 一致。本次试运行已附带 LIVE_CHECK=1 实测。
