...
| C5 引用有效 | ⚠️ | 核心页面通过；遗留页面 ['formula_analysis_v2.html'] 仍含旧引用（技术债，参考 MIGRATION.md，建议改 SSOT 或下线） |
...
| P3 基础回报公式 | ⚠️ | 两页均含「税收溢价 + 250/750 移动平均」核心口径，建议统一措辞 |
...
| F2 BP 单位 | ⚠️ | index.html 出现小写 bp/bps；formula_report.html 出现小写 bp/bps；trigger_analysis_report.html 使用「个基点」（与全站 BP 不统一）；trigger_analysis_report.html 出现小写 bp/bps（建议统一为 BP；当前不阻断） |
...
| A2 数据不过期 | ✅ | last_updated=2026-07-10 13:51（0.0 天前，≤40） |
| A3 发版后实测 | ✅ | 线上两页无旧值、均引用 predictions.json，predictions.json 首值=1.9313 |

| **整体结论** | **⚠️ 通过（6 项提示，需关注）** | FAIL=0, WARN=6 |
